...
Expect multiply ("a", 2) to be error
...
expect unexpected input to be an error
expect unexpected input to be strings or booleans
expect Multiples to not repeat but show up as a single value
Functional Tests:
...
When a user checksout as guest], prompted to create an account
When a user checksout with empty cart, they should be suggested items to put in
When a user checksout while logged in, their address and payment info should be recalled.
When a user checksout while logged in, they could be given a discount or asked if they have a discount code
Collapse
...
Message Skylar Rogers, Song Kim
